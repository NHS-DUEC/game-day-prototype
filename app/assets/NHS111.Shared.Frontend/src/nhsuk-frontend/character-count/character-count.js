...
CharacterCount.prototype.updateVisibleCountMessage =
  function updateVisibleCountMessage() {
    const { $textarea } = this;
...
CharacterCount.prototype.updateScreenReaderCountMessage =
  function updateScreenReaderCountMessage() {
...
CharacterCount.prototype.formattedUpdateMessage =
  function formattedUpdateMessage() {
    const { $textarea } = this;
    const { options } = this;
    const remainingNumber = this.maxLength - this.count($textarea.value);
...
export default () => {
  const characterCounts = Array.from(
    document.querySelectorAll('[data-module="nhsuk-character-count"]')
  );
