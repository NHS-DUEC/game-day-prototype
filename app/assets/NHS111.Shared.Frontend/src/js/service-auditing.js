...
const getServiceData = (service, dosData) => {
  ...
  let serviceId = service.getAttribute("data-visible-service-id");
  let serviceOfferedElement = service.querySelector("[data-services-offered]");
  let dosDataInput = getKeyByValue(dosData, serviceId);
  let serviceInputId = dosDataInput.substring(0, dosDataInput.lastIndexOf("."));
  let serviceItemData = {};
  let isValidationService = service.hasAttribute("data-validation-service");
...
const setEventData = (services, serviceItemsData) => {
  ...
  let filteredServices = serviceItemsData.filter(
  ...
  if (serviceActions.length > 0) {
    serviceActions.forEach((action) => {
      if (!action.hasAttribute("Hidden")) {
        let actionValue = action.getAttribute("data-service-action");
